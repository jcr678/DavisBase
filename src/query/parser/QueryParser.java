...
public class QueryParser {
    ...
    public static IQuery ShowTableListQueryHandler() {
        if(QueryParser.CurrentDatabaseName.equals("")){
            System.out.println(QueryParser.NO_DATABASE_SELECTED_MESSAGE);
            return null;
        }

        return new ShowTableQuery(QueryParser.CurrentDatabaseName);
    }
    ...
    public static IQuery DropTableQueryHandler(String tableName) {
        ...
        return new DropTableQuery(QueryParser.CurrentDatabaseName, tableName);
    }
    ...
    public static IQuery SelectQueryHandler(String[] attributes, String tableName, String conditionString) {
        ...
        if(conditionString.equals("")){
            query = new SelectQuery(QueryParser.CurrentDatabaseName, tableName, columns, null, isSelectAll);
            return query;
        }
        ...
        query = new SelectQuery(QueryParser.CurrentDatabaseName, tableName, columns, conditionList, isSelectAll);
        return query;
    }
    ...
    public static IQuery InsertQueryHandler(String tableName, String columnsString, String valuesList) {
        ...
        if(columns != null && columns.size() != values.size()){
            QueryParser.UnrecognisedCommand("", "Number of columns and values don't match");
            return null;
        }

        query = new InsertQuery(QueryParser.CurrentDatabaseName, tableName, columns, values);
        return query;
    }
    ...
    public static IQuery DeleteQueryHandler(String tableName, String conditionString) {
        ...
        if(conditionString.equals("")){
            query = new DeleteQuery(QueryParser.CurrentDatabaseName, tableName, null);
            return query;
        }
        ...
        query = new DeleteQuery(QueryParser.CurrentDatabaseName, tableName, conditions);
        return query;
    }
    ...
    public static IQuery UpdateQuery(String tableName, String clauseString, String conditionString) {
        ...
        if(clause.operator != Operator.EQUALS){
            QueryParser.UnrecognisedCommand(clauseString, "SET clause should only contain = operator");
            return null;
        }

        if(conditionString.equals("")){
            query = new UpdateQuery(QueryParser.CurrentDatabaseName, tableName, clause.column, clause.value, null);
            return query;
        }
        ...
        query = new UpdateQuery(QueryParser.CurrentDatabaseName, tableName, clause.column, clause.value, condition);
        return query;
    }
    ...
    public static IQuery CreateTableQueryHandler(String tableName, String columnsPart) {
            ...
            if (columnsList[i].toLowerCase().endsWith("primary key")) {
                if (i == 0) {
                    if (columns.get(i).type == DataType.INT) {
                        ...
                        QueryParser.UnrecognisedCommand(columnsList[i], "PRIMARY KEY has to have INT datatype");
                        return null;
                    }
                }
                else {
                    QueryParser.UnrecognisedCommand(columnsList[i], "Only first column should be PRIMARY KEY and has to have INT datatype.");
                    return null;
                }
        ...
        query = new CreateTableQuery(QueryParser.CurrentDatabaseName, tableName, columns, hasPrimaryKey);
        return query;
    }
    ...
    public static boolean IsDatabaseExists(String databaseName) {
        ...
        if (databaseName == null || databaseName.length() == 0) {
            QueryParser.UnrecognisedCommand("", USE_HELP_MESSAGE);
            return false;
        }
    ...
    public static IQuery DescTableQueryHandler(String tableName) {
        ...
        return new DescTableQuery(QueryParser.CurrentDatabaseName, tableName);
    }
}
